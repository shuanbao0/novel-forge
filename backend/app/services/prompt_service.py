...
⚠️ 若【章节内容】中出现"⋯⋯此处省略约 N 字⋯⋯"分隔符，说明这是长章节的首尾+中段采样，请将其视为同一章节的局部样本进行分析，不要将省略视为"叙事跳跃"或"内容缺失"扣分。
</chapter>

<chapter_intent priority="P0">
【本章创作意图（大纲/章级契约预设）】
以下是作者/大纲对本章原本的预设。请把它当作"评估基准"——评分与建议必须围绕"实际成稿是否实现了这些意图"展开，而不是脱离意图给出通用的写作建议。

{chapter_intent}

评估指引：
- 若关键事件全部落实且自然，pacing/coherence 评分相应提升
- 若关键事件遗漏、走形或被冗余支线挤压，必须在 suggestions 中明确指出"漏写了/偏离了哪条预设"
- 若实际成稿超出预设（合理拓展），不算扣分；若与预设冲突（如情感基调反向、角色焦点错位），必须在 suggestions 中提示
</chapter_intent>

<recent_context priority="P1">
【最近章节回顾 - 用于节奏与连贯性判断】
以下是本章之前 1-3 章的摘要。请基于这些上下文判断：
- 本章节奏是否承接前文（前章已快节奏推进，本章是否需要喘息？反之亦然）
- 是否存在重复信息（前章已交代的设定/情节，本章不应再次大段复述）
- 角色情绪/状态衔接是否自然
- 钩子设置是否与前文回收形成呼应
...
<upcoming_plan priority="P1">
【后续章节走向 - 用于伏笔与铺垫合理性判断】
以下是本章之后 1-2 章的大纲梗概。请基于这些预期判断：
- 本章埋下的新伏笔，是否能在已规划的后续章节中找到回收点（若完全无关，标记 strength 偏低）
- 本章是否漏掉了"为下章做铺垫"的必要动作（应在 suggestions 中提示）
- 不要建议添加与后续走向冲突的情节方向

{upcoming_outline}
</upcoming_plan>

<score_baseline priority="P1">
【历史评分基线 - 用于评分校准与避免凑数建议】
{score_baseline}

评分校准规则（强制）：
- 评分必须先与基线对照，再给出本章分数。例如：基线 overall=7.2，本章如果实质相当，应落在 6.8-7.6 区间，而不是凭印象给 8.0
- 若本章评分偏离基线 ≥1.0（高或低），必须在 score_justification 中明确说明"相比前 N 章基线 X.X，本章因 YYY 升高/降低"
- 不要让评分无锚漂移；同一作者的相邻章节质量分布通常稳定
</score_baseline>
...
🔴 根本原则：建议条数由"实际问题数量"决定，不由"凑够规定条数"决定。宁可少而精，不可多而泛。

**有基线时（score_baseline 提供了 sample_size ≥ 2）**：
- 本章 overall 显著低于基线（差 ≥1.0）：必须 3-5 条建议，指出导致下滑的具体问题
- 本章 overall 接近基线（差 <1.0）：仅在确实发现"实质问题"时给建议，0-3 条，质量优先
- 本章 overall 显著高于基线（差 ≥1.0）：通常 0-1 条；可指出"可继续保持"的亮点，不为凑数硬找问题
- 若【高频建议标签 Top N】中的老问题在本章再次出现，必须明确标注"老问题复发"
- 若【高频建议标签 Top N】中的老问题在本章已解决，可在 suggestions 中减免重复提示

**无基线时（前期章节，sample_size < 2）**：
- 仅按"实际问题严重程度"给建议：严重 3-5 条，明显 1-3 条，轻微 0-1 条
- 不要因为"按规则要给 N 条"而硬编不存在的问题

每条建议必须：
- 以 `【标签】` 开头（如 `【节奏问题】`、`【伏笔缺失】`），便于后续归类与基线统计
- 指出具体问题位置或类型
- 说明为什么是问题（参照基线、参照本章意图、或参照客观写作标准）
- 给出明确的改进方向（"删什么/加什么/改成什么"，避免只说"建议优化"）
</analysis_framework>
...
❌ 使用 markdown 或代码块
    ...
    # AI 去味提示词 - 将 AI 生成文本改写为更具人类作家手笔的版本
    AI_DENOISING = """<system>
你是一位资深中文网文编辑兼改稿手,擅长把"AI 痕迹明显"的文本改写为"人类作家手笔"的版本。
你只输出改写后的成文,绝不输出任何解释、前后缀或元信息。
</system>

<task>
【任务】
对下方"原始文本"进行 AI 去味改写。在保留原文情节、人物、视角与时序的前提下,消除 AI 风格,提升真实感与可读性。

【改写原则】
1. 去除模板化副词:不使用"缓缓/淡淡/微微/默默/轻轻/慢慢"等;改用具体动作或环境暗示。
2. 去除情绪标签:不要写"他很愤怒""她感到悲伤";用动作、生理反应、对话节奏表现情绪。
3. 去除万能形容词:"美丽的/神秘的/强大的"要落到具体感官细节(如:左手三道旧疤)。
4. 去除 AI 套路开头与结尾抒情总结:不写"夜幕降临""阳光洒下"等;结尾留悬念、动作未完或对话被打断。
5. 让对话生活化:口语化、有个性差异、允许打断与不完美。
6. 让节奏松弛:允许短句、半句、思维跳跃;不要一味工整对仗。
7. 让动作有物理细节:谁、用哪只手、什么角度、对方什么反应,而不是抽象的"他攻击她"。
8. 保持原文的人称、时态、主要情节与人物设定,不增不减关键事实。
</task>
    ...
    def get_all_system_templates(cls) -> list:
        ...
        template_definitions = {
            ...
            "AI_DENOISING": {
                "name": "AI去味",
                "category": "AI去味",
                "description": "将AI生成的文本改写得更像人类作家的手笔，去除模板化痕迹",
