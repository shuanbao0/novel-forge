...
# ── 路径配置 ──────────────────────────────────────────────────────────────────
INSTALL_DIR="$HOME/novel-forge"                    # 项目安装目录
DATA_DIR="$HOME/novel-forge/data"                   # 数据库目录
LOG_DIR="$HOME/novel-forge/logs"                    # 日志目录
REPO="${NOVEL_FORGE_REPO:-https://github.com/your-org/novel-forge.git}"  # 通过环境变量 NOVEL_FORGE_REPO 覆盖
...
echo ""
echo -e "${CYAN}╔══════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║   📚 novel-forge Termux 一键安装         ║${NC}"
echo -e "${CYAN}╚══════════════════════════════════════════╝${NC}"
echo ""
...
python3 << 'PYEOF'
import os
f = os.path.expanduser("~/novel-forge/backend/app/services/memory_service.py")
with open(f) as fh:
    c = fh.read()
...
files = [
    f"{home}/novel-forge/backend/app/api/chapters.py",
    f"{home}/novel-forge/backend/app/api/memories.py",
    f"{home}/novel-forge/backend/app/api/outlines.py",
    f"{home}/novel-forge/backend/app/api/projects.py",
    f"{home}/novel-forge/backend/app/services/foreshadow_service.py",
]
...
# novel-forge Termux 配置
APP_NAME=novel-forge
APP_HOST=0.0.0.0
APP_PORT=8000
DEBUG=false
TZ=Asia/Shanghai
...
DATABASE_URL=sqlite+aiosqlite:///data/data/com.termux/files/home/novel-forge/data/ai_story.db

# 日志
LOG_LEVEL=INFO
LOG_TO_FILE=true
LOG_FILE_PATH=/data/data/com.termux/files/home/novel-forge/logs/app.log
...
# =============================================================================
# 步骤 9: 创建启动脚本
# 说明: 生成 ~/novel-forge-start.sh，支持前台/后台运行
# =============================================================================
step 9 $TOTAL "创建启动脚本"
cat > "$HOME/novel-forge-start.sh" << STARTEOF
#!/bin/bash
# novel-forge Termux 启动脚本
set -e
...
if [ "\$1" = "--bg" ]; then
    echo "🚀 后台启动 novel-forge (端口 8000)..."
    nohup "\$PYTHON" -m uvicorn app.main:app --host 0.0.0.0 --port 8000 \\
        > "\$LOG_DIR/app.log" 2>&1 &
    echo \$! > "$HOME/novel-forge.pid"
    sleep 2
    if kill -0 \$(cat "$HOME/novel-forge.pid") 2>/dev/null; then
        echo "✅ 已启动, PID: \$(cat $HOME/novel-forge.pid)"
    else
        echo "❌ 启动失败，查看日志: \$LOG_DIR/app.log"
        exit 1
    fi
else
    echo "🚀 启动 novel-forge (端口 8000, Ctrl+C 停止)..."
    exec "\$PYTHON" -m uvicorn app.main:app --host 0.0.0.0 --port 8000
fi
STARTEOF
chmod +x "$HOME/novel-forge-start.sh"
info "启动脚本已创建: ~/novel-forge-start.sh"
...
echo ""
echo -e "${GREEN}╔══════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║  🎉 novel-forge 安装完成！                   ║${NC}"
echo -e "${GREEN}╠══════════════════════════════════════════════╣${NC}"
echo -e "${GREEN}║                                              ║${NC}"
echo -e "${GREEN}║  前台运行（Ctrl+C 停止）:                     ║${NC}"
echo -e "${GREEN}║    bash ~/novel-forge-start.sh                ║${NC}"
echo -e "${GREEN}║                                              ║${NC}"
echo -e "${GREEN}║  后台运行:                                    ║${NC}"
echo -e "${GREEN}║    bash ~/novel-forge-start.sh --bg           ║${NC}"
echo -e "${GREEN}║                                              ║${NC}"
echo -e "${GREEN}║  停止后台:                                    ║${NC}"
echo -e "${GREEN}║    kill \$(cat ~/novel-forge.pid)              ║${NC}"
echo -e "${GREEN}║                                              ║${NC}"
echo -e "${GREEN}║  查看日志:                                    ║${NC}"
echo -e "${GREEN}║    tail -f ~/novel-forge/logs/app.log         ║${NC}"
echo -e "${GREEN}║                                              ║${NC}"
